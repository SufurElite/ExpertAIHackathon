...
def loadModel():
    import h5py
    gcs_source_uri = "gs://chadvice.appspot.com/models/interestModel.h5"
    ...
    prefix2 = "models/interestModel.json"
    ...
    print(blob2.name) 
    print(blob3.name) 
    blob.download_to_filename(os.getcwd()+"/interestModel.h5")
    blob2.download_to_filename(os.getcwd()+"/interestModel.json")
    ...
    os.remove(os.getcwd()+"/interestModel.h5")
    os.remove(os.getcwd()+"/interestModel.json")
...
def loadModelLocal():
    ...
    json_file = open('interestModel.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model = model_from_json(loaded_model_json)
    # load weights into new model
    loaded_model.load_weights("interestModel.h5")
...
def loadOnlyLocal():
    # load json and create model
    json_file = open('model_files/interestModel.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model = model_from_json(loaded_model_json)
    # load weights into new model
    loaded_model.load_weights("model_files/interestModel.h5")
    print("Loaded model from disk")
